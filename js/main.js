$(document).ready(function(){
    //e2fe09992599f67b7870de2796562c70
    //https://api.openweathermap.org/data/2.5/weather?q=busan&appid=e2fe09992599f67b7870de2796562c70

    var cityName = []; //버튼 클릭시 또는 검색바에서 입력 후 엔터를 누를 시, 도시명 데이터를 수집한다.
    console.log(cityName); //[]

    var myAPI = `e2fe09992599f67b7870de2796562c70`;
    ...
    function w_info(){
        ...
        //append(), prepend() 자식을 추가하는 메서드는 기존 자식을 남기는 상태에서 새로운 자식들을 추가 ==> 기존 데이터를 새롭게 반복하여 적용하고자 할때는 반복문 이전에 초기화가 필요
    ...
    $(document).on("click", ".cities button", function(){
        console.log(this);
        var city_txt = $(this).text(); //클릭한 도시의 텍스트를 추출
        console.log(city_txt);

        cityName.unshift(city_txt); //cityName이라는 전역변수 배열 데이터의 첫번째 인덱스에 추출한 텍스트(도시명)을 넣어준다.
        console.log(cityName);

        $(this).remove(); //클릭한 도시의 버튼을 제거한다.
    ...
    navigator.geolocation.getCurrentPosition(function(position){
        ...
        var latitude = position.coords.latitude; //latitude 위도
        var longitude = position.coords.longitude; //longitude 경도
